...
int limR[maxn][2], limC[maxn][2], LR[maxn], LC[maxn];
int mp[510][2];
...
int main()
{
    ...
    for(int i = 0; i < m; i ++)
    {
        ...
        if(op[0] == 'R') limR[cntR][0] = pos, limR[cntR++][1] = k;
        else if(op[0] == 'C') limC[cntC][0] = pos, limC[cntC++][1] = k;
    }
    posR.push_back(0);
    posC.push_back(0);
    ...
    {
        pos = lower_bound(posR.begin(), posR.end(), limR[i][0]) - posR.begin();
        LR[pos] = min(LR[pos], limR[i][1]);
    }
    for(int i = 0; i < cntC; i ++)
    {
        pos = lower_bound(posC.begin(), posC.end(), limC[i][0]) - posC.begin();
        LC[pos] = min(LC[pos], limC[i][1]);
    }
