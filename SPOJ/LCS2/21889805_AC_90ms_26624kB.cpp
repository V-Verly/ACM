...
struct SuffixAutomation
{
    ...
    int sz[maxn << 1], a[maxn << 1], c[maxn << 1], mn[maxn << 1], mx[maxn << 1];
	...
	void solve(char s2[])
	{
        ...
        */
        ...
        for(int i = 0; s2[i]; i ++)
        {
            ...
            mx[p] = max(mx[p], now_len);
        }
        for(int i = cnt; i >= 1; i --)
        {
            int x = a[i];
            mn[x] = min(mn[x], mx[x]);
            if(mx[x] && fa[x]) mx[fa[x]] = len[fa[x]];
            mx[x] = 0;
        }
    }
...
int main()
{
    ...
    sam.Toposort();
    while(scanf("%s", t) != EOF) sam.solve(t);
    int res = 0;
    for(int i = 2; i <= sam.cnt; i ++) res = max(res, sam.mn[i]);
    printf("%d\n", res);
    return 0;
}
